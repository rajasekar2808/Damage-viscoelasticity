...
class viscoElasticity2dPlaneStrain():
    ...
    def __init__(self, n_i=2, lamb_i = (1.,1.), mu_i =(1.,1.), tau_i = (0.1,),Yc =1.,Gc=1., g1 = GQuadratic(), g2=G_Const_1(), h =HQuadratic(),var_con = True):
        ...
        self.Gc = Gc
        if g1.name != g2.name:
            print('Implementation for g1!=g2 . Might fail if g(d=1) =0')
            ## could be resovled by maping d = 1 to d = .99999999999.. if g1(d=1)=0 and g2(d=1)=0
    ...
    def driving_force(self,strain,eps_i_np1,eps_i_n,DT):
        ## driving force for phase field simulation AT2 
        ## (IMP: ensure g1  ~ (1-d)^2 for AT2   ::   g2 ~ 1 or (1-d)^2)
        
        if not self.var_con or self.g2(1)==1:
            ## self.g2(1)==1 indicates g2(d) =1 (a constant for all d)
    ...
    def fe(self,strain,eps_i, d):
        ...
        return self.g1(d) * self._free_energy_potential(strain, eps_i)
    
    def fv(self, eps_i_np1, eps_i_n, DT,d):
        ## return incrementla viscous dissipation b/w t_n and t_n+1
        return 2*DT*self.g2(d) * self._visc_dissipation_potenial(eps_i_np1, eps_i_n, DT)
    ...
    def potentialFixedStrain(self, strain ,eps_i_np1,eps_i_n=None,DT=None):
        ...
        def phid_2(d, phi = True, Y = False, dY = False):
            ...
            if phi : res['phi']  =  g1(d)*(phie) + g2(d)*(DT*phiv) + Yc*h(d)
            if Y   : res['Y']    = -g1.jac(d)*(phie) - g2.jac(d)*(DT*phiv)    - Yc * h.jac(d)
            if dY  : res['dY']   = -g1.hess(d)*(phie) - g2.hess(d)*(DT*phiv)      - Yc * h.hess(d)
            return res
        ...
        def phid_1(d, phi = True, Y = False, dY = False):
            ...
            if phi : res['phi']  =  g1(d)*(phie)  + Yc*h(d)
            if Y   : res['Y']    = -g1.jac(d)*(phie)     - Yc * h.jac(d)
            if dY  : res['dY']   = -g1.hess(d)*(phie)      - Yc * h.hess(d)
    ...
    def H_matrix(self,DT,d=None):
        ...
        if g1.name == g2.name:
            ##  Hooks tensor same for all element (bcoz fo effect stress princip)
            Htmp = np.eye(3)
            for i in range(1,self.ni):
            ...
            ## H[i] is the Hooks tensor of element i
            if d is None:
                raise('Damage required')
            Htmp = np.tensordot(np.ones(len(d)), np.eye(3), axes=0)
            for i in range(1,self.ni):
                k = g1(d)*DT/(DT*g1(d) + tau[i-1]*g2(d))
                H0i = np.dot(H0,self.invHi['H'+str(i)])
                Htmp +=  k[:,np.newaxis,np.newaxis]*H0i
            H = np.linalg.inv(Htmp)*H0
            return H
    ...
    def solve_stress_eps1(self, eps, eps_i, DT,d, find_eps_i=True):
        ...
        if g1.name == g2.name:
            ...
            for i in range(1,self.ni):
                k = (g2(d)*tau[i-1]/(g1(d)*DT+tau[i-1]*g2(d)))
                eps_eff +=  -k[:,np.newaxis]* eps_i[:,i*3:(i+1)*3]
            ...
            if find_eps_i:
                ...
                for i in range(1,self.ni):
                    invHi = self.invHi['H'+str(i)]
                    temp_var = np.empty((ne,3))
                    temp_var[:,0] = invHi[0,0]*stresses_ef[:,0] + invHi[0,1]* stresses_ef[:,1]
                    temp_var[:,1] = invHi[1,0]*stresses_ef[:,0] + invHi[1,1]* stresses_ef[:,1]
                    temp_var[:,2] = invHi[2,2]* stresses_ef[:,2]
                    
                    k1 = DT/(g1(d)*DT+ g2(d)*tau[i-1])
                    k2 = g2(d) * tau[i-1]/ (DT*g1(d) + g2(d)*tau[i-1])
                    
                    eps_i_np1[:,i*3:(i+1)*3] = k1[:,np.newaxis] * temp_var + k2[:,np.newaxis] * eps_i[:,i*3:(i+1)*3]
                return stresses_ef.squeeze(), eps_i_np1.squeeze()
    ...
    def dTrialStressDStrain(self, nf, DT, d, asym_para = None):
        ...
        g2 = self.g2
        if g1.name == g2.name:
            return np.tensordot( g1(d) ,  H, axes = 0 ).squeeze()
        else:
            return g1(d)[:,np.newaxis,np.newaxis] * H
    ...
    def trialStress(self, strain, eps_i, d = None,visc_param = None):
        ...
        if d is not None: return self.g1(d).reshape(len(d),1)*tr_stress
